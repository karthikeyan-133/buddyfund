...
import { ContributionRecord, Circle, User as UserType } from '../types';
...
export const SavingsTab: React.FC<SavingsTabProps> = ({
  ...
  // Distinct weeks
  const availableWeeks = Array.from(new Set(contributions.map((c) => c.weekNumber))).sort(
    (a: number, b: number) => a - b
  );

  // Filtered contributions
  const filtered = contributions.filter((item) => {
    if (selectedWeek !== 'all' && item.weekNumber !== selectedWeek) return false;
    if (selectedStatus !== 'all' && item.status !== selectedStatus) return false;
    if (
      searchMember &&
      !item.userName.toLowerCase().includes(searchMember.toLowerCase())
    )
      return false;
    return true;
  });
  ...
  // Calculate totals
  const totalExpectedAll = contributions.reduce((sum, c) => sum + c.amount, 0);
  const totalCollectedAll = contributions.reduce((sum, c) => sum + c.paidAmount, 0);
  ...
  const handleQuickPaySubmit = (e: React.FormEvent) => {
    ...
    onRecordPayment({
      ...
      weekNumber: quickPayRecord.weekNumber,
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h4 className="font-bold text-slate-900 text-sm">{item.userName}</h4>
                    <span className="text-[11px] text-slate-500 font-medium">{item.weekLabel}</span>
                  </div>
                </div>
          ...
          <table className="w-full text-left text-xs sm:text-sm">
            <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 font-semibold uppercase text-[11px] tracking-wider">
              <tr>
                ...
                <th className="py-3 px-4">Week &amp; Due Date</th>
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/60 transition">
                    ...
                    <td className="py-3 px-4">
                      <div className="font-medium text-slate-800">{item.weekLabel}</div>
          ...
          <div className="bg-white rounded-t-3xl sm:rounded-2xl shadow-2xl border border-slate-200 max-w-md w-full p-5 sm:p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  {quickPayRecord.userName} • {quickPayRecord.weekLabel}
                </p>
              </div>
              <button
            ...
            <form onSubmit={handleQuickPaySubmit} className="space-y-4 text-xs sm:text-sm">
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g. GPay UPI Ref 928174 or Cash collected by Karthik"
                  value={refNote}
