...
interface HeaderProps {
  ...
  onOpenEditCircle?: (circle: Circle) => void;
  onOpenDeleteCircle?: (circle: Circle) => void;
    ...
    <header className="sticky top-0 z-40 bg-white/98 backdrop-blur-md border-b border-slate-200 shadow-sm w-full">
      ...
      <div className="sm:hidden px-3 pt-2.5 pb-2 flex flex-col gap-2">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <button
            ...
            className="flex items-center text-left group focus:outline-none flex-shrink-0"
            title="Ithanu_njangal Home"
          >
            <Logo size="sm" subtitle="Squad Savings" />
          </button>
      ...
      <div className="hidden sm:flex max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-1.5 sm:gap-3 min-w-0">
          <button
            ...
            title="Ithanu_njangal Home"
          >
            <Logo size="md" subtitle="Savings &amp; Tour Ledger" />
          </button>
        ...
        <div className="flex items-center gap-1.5 sm:gap-3 flex-shrink-0">
          ...
          <div className="relative">
            <button
              ...
              title={`Signed in as ${currentUser.name} (${currentUser.role.replace('_', ' ')}) — Click to view profile or switch user`}
            >
              <div className="relative flex-shrink-0">
                <img
                  src={currentUser.avatarUrl}
      ...
      {showCircleMenu && (
        <>
          ...
          />
          <div className="fixed sm:absolute left-3 right-3 sm:left-44 sm:right-auto top-[95px] sm:top-14 sm:w-72 bg-white rounded-2xl sm:rounded-xl shadow-2xl border border-slate-200 py-2 z-50 animate-in fade-in zoom-in-95 duration-150">
            ...
            {circles.length === 0 ? (
              ...
              circles.map((c) => {
                const isSelected = currentCircle?.id === c.id;
                const isCircleAdmin = currentUser.role === 'circle_admin' || currentUser.role === 'super_admin';

                return (
                  <div
                    key={c.id}
                    className={`w-full px-3 py-2 text-left text-sm flex items-center justify-between hover:bg-emerald-50/80 transition-colors group ${
                  ...
                  >
                    ...
                    {/* Circle Admin Actions: Edit & Delete */}
                    {isCircleAdmin && (
                      <div className="flex items-center gap-1 flex-shrink-0">
                        <button
                          ...
                          title={`Edit ${c.name}`}
                          className="p-1.5 text-slate-400 hover:text-emerald-700 hover:bg-white rounded-lg transition border border-transparent hover:border-emerald-200 shadow-none hover:shadow-xs"
                        >
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            setShowCircleMenu(false);
                            onOpenDeleteCircle?.(c);
                          }}
                          title={`Delete ${c.name}`}
                          className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-white rounded-lg transition border border-transparent hover:border-rose-200 shadow-none hover:shadow-xs"
                        >
            ...
            <div className="border-t border-slate-100 mt-2 pt-2 px-3 space-y-1">
              {onSignOut && (
                <button
                  ...
                  className="w-full py-2 px-3 text-xs text-rose-600 hover:text-rose-700 hover:bg-rose-50 rounded-xl font-bold flex items-center justify-center gap-2 transition"
                >
              ...
              <button
                ...
                className="w-full py-1.5 px-3 text-[11px] text-slate-500 hover:text-slate-800 hover:bg-slate-50 rounded-lg text-center font-medium transition"
              >
