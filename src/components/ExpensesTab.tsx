...
export const ExpensesTab: React.FC<ExpensesTabProps> = ({
  ...
  const largestCategoryEntry = (Object.entries(categoryTotals) as [string, number][]).sort(
    (a, b) => b[1] - a[1]
  )[0];
  const largestCategory: string = largestCategoryEntry ? largestCategoryEntry[0] : 'None';
  const largestCategoryAmount: number = largestCategoryEntry ? largestCategoryEntry[1] : 0;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
          ...
          <div className="text-xl font-bold text-purple-700 mt-1 truncate">
            {largestCategory}
          </div>
          <p className="text-[11px] text-slate-400 mt-0.5">
            {largestCategoryAmount > 0 ? `${currency}${largestCategoryAmount.toLocaleString()} total spend` : 'No spend recorded'}
          </p>
        </div>
