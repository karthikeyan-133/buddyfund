...
import { Transaction, Circle, TransactionType, User, CircleMember } from '../types';
...
export const LedgerTab: React.FC<LedgerTabProps> = ({
  ...
  // Resolve member name reliably from members roster, localStorage, or transaction data
  const resolveMemberName = (tx: Transaction) => {
    // 1. If tx.memberId exists, lookup in circle members roster
    if (tx.memberId) {
      const found = members.find((m) => m.userId === tx.memberId || m.id === tx.memberId);
      if (found?.name && found.name !== 'Squad Member' && found.name !== 'Member') {
        return found.name;
      }
    }
    // 2. If tx.memberName exists and is an actual name
    if (tx.memberName && tx.memberName !== 'Squad Member' && tx.memberName !== 'Member') {
      return tx.memberName;
    }
    // 3. If tx.memberId matches current user
    if (tx.memberId === currentUser.id && currentUser.name) {
      return currentUser.name;
    }
    // 4. Try localStorage saved members
    try {
      const savedMembers = JSON.parse(localStorage.getItem('buddyfund_members') || '[]');
      const localFound = savedMembers.find((m: any) => m.userId === tx.memberId || m.id === tx.memberId);
      if (localFound?.name && localFound.name !== 'Squad Member' && localFound.name !== 'Member') {
        return localFound.name;
      }
    } catch {}

    return tx.memberName && tx.memberName !== 'Squad Member'
      ? tx.memberName
      : (tx.category || tx.type.replace(/_/g, ' ') || 'Squad Member');
  };

  // Filtered
  const filtered = transactions.filter((t) => {
    if (selectedType !== 'all' && t.type !== selectedType) return false;
    const memberDisplay = resolveMemberName(t);
    if (
      searchTerm &&
      !t.notes.toLowerCase().includes(searchTerm.toLowerCase()) &&
      !t.reference.toLowerCase().includes(searchTerm.toLowerCase()) &&
      !memberDisplay.toLowerCase().includes(searchTerm.toLowerCase())
    )
      return false;
    return true;
  });
