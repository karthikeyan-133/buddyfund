...
import { Circle, ContributionRecord, Expense, Loan, Tour, Transaction, User as UserType } from '../types';
import { printProfessionalReport } from '../utils/reportPrinter';
...
export const ReportsTab: React.FC<ReportsTabProps> = ({
  ...
  const handleDownloadReportCSV = () => {
    ...
    if (reportType === 'statement') {
      ...
      headers = ['Member', 'Date', 'Expected', 'Paid', 'Status', 'Payment Method'];
      rows = contributions.map((c) => [c.userName, c.dueDate || c.paidDate || c.weekLabel, c.amount, c.paidAmount, c.status, c.paymentMethod || '']);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 no-print print:hidden">
        ...
        <div className="grid grid-cols-2 sm:flex items-center gap-2 w-full sm:w-auto">
          <button
            onClick={handlePrint}
            className="min-h-[44px] justify-center px-3.5 py-2 bg-slate-900 hover:bg-slate-800 text-white text-xs sm:text-sm font-semibold rounded-xl transition flex items-center gap-1.5 shadow-sm active:scale-95 text-center"
            title="Generate Professional Printable Financial Report"
          >
      ...
      {/* Tabs */}
      <div className="flex flex-nowrap overflow-x-auto no-scrollbar gap-2 border-b border-slate-200 pb-2.5 text-xs sm:text-sm font-medium -mx-4 px-4 sm:mx-0 sm:px-0 no-print print:hidden">
        {[
                ...
                {contributions.length === 0 ? (
                  ...
                  contributions.map((c) => (
                    <tr key={c.id}>
                      ...
                      <td className="py-2.5 px-3">{c.dueDate || c.paidDate || c.weekLabel}</td>
                ...
                {loans.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="py-6 text-center text-slate-400">
                      No internal loans in portfolio.
                    </td>
                  </tr>
                ) : (
                  loans.map((l) => (
      ...
      {reportType === 'tours' && (
        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm space-y-4">
          ...
          {tours.length === 0 ? (
            <p className="py-6 text-center text-slate-400 text-xs">No tours planned yet.</p>
          ) : (
            tours.map((t) => (
